...
def main() -> None:
    output_dir = ensure_dir(OUTPUTS_DIR / "effnet_b0_head")
    ...
    model = TransferClassifier(backbone_name="efficientnet_b0", num_classes=2, freeze_backbone=True).to(DEVICE)
    ...
    lines = [
        ...
        "",
        "Best-practice setup:",
        "- EfficientNet-B0 pretrained backbone frozen",
        "- Train only classification head",
        "- AdamW optimizer over model.head parameters",
        "",
        "Test metrics:",
    ...
    save_text_report(output_dir / "effnet_b0_head_report.txt", lines)
    save_json(output_dir / "effnet_b0_head_history.json", {"history": history, "test": details})
